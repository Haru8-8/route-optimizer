...
#### OR-Tools（厳密解法）
Google OR-Tools の Routing Library を使い、Branch & Bound で最適解を保証する。
...
|---|---|---|
| 解の保証 | 最適解を保証 | 保証なし（良好な近似解） |
| 小規模（〜20点） | 高速・最適 | OR-Toolsより速いが解の質は劣る |
...
├── solver/
│   ├── tsp_ortools.py          # TSP OR-Tools厳密解法
│   ├── tsp_greedy2opt.py       # 貪欲法 + 2-opt
│   ├── tsp_sa.py               # 焼きなまし法
│   ├── vrp_ortools.py          # VRP OR-Tools厳密解法
│   ├── vrp_clarke_wright.py    # Clarke-Wright節約法
│   ├── vrptw_ortools.py        # VRPTW OR-Tools厳密解法
